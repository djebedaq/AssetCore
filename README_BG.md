...
Оперативният машинен статус използва само стабилните кодове `READY`, `ISSUED` и `REPAIR`. Подробните етапи `DIAGNOSIS`, `WAITING_PARTS`, `REPAIRING`, `TESTING` и други са част от ремонтната карта. Човешките имена се визуализират според избрания език; историческите audit/document snapshots не се пренаписват.
...
Seed-ът регистрира машинно използваеми BG/EN/RU DOCX източници с SHA-256 и успешен validation report. Repair protocol v5 е един общ тричастов комплект; всеки езиков шаблон съдържа собствен текст и няма автоматичен машинен превод на официален документ. Reference снимките и историческите DOCX/PDF остават контролирани read-only източници и не се използват като нови бизнес записи.

Ремонтът се управлява с „Запази“ и ясното действие „Запази и продължи“ за следващия нормален етап. `WAITING_APPROVAL` и `WAITING_PARTS` са незадължителни разклонения. Полетата се изискват при съответния преход, а server връща конкретната липсваща информация на български. Реалният труд се записва отделно като минути за диагностика, ремонт и тестване; периодът между отваряне и приключване не се представя като работно време.

Ремонтът може да стане `COMPLETED` и машината — `READY` само след записан преглед, изискано почистване, диагностика, необходима и извършена работа, успешен тест, краен резултат и успешно създаден задължителен ремонтен DOCX/PDF. Документите, `COMPLETED`, `READY` и активното местоположение `Цех` се записват атомарно. Грешка в генератора връща цялата транзакция и оставя машината `REPAIR`. След успешен commit машината е налична за ново издаване без допълнителна ръчна стъпка. Правилото е backend-authoritative и важи и за стария съвместим маршрут.
