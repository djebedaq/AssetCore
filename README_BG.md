...
От паспорта може директно да се отвори machine-first каталогът. Операторът избира възел, увеличава/премества оригиналната exploded схема и натиска проверена позиция или избира ред от пълната source таблица. Една позиция може да покаже няколко `Valid for` variants без сливане. Единичната част влиза в количката с заявено количество 1, отделно от количеството по схема; source-backed repair kit добавя всички свои проверени компоненти и поддържа Undo. Оригиналният и `Replaced by` Part No. се показват и остават търсими.
