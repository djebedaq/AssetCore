...
Ремонтът може да стане `COMPLETED` и машината — `READY` само след записан преглед, изискано почистване, извършена работа, успешен тест, краен резултат и успешно създаден задължителен ремонтен DOCX/PDF. Грешка в генератора връща цялата транзакция и оставя машината `REPAIR`. Правилото е backend-authoritative и важи и за стария съвместим маршрут.
...
Ремонтите са вътрешни: приключващият регистриран потребител се записва
автоматично и се създава заключен repair DOCX/PDF без роли „предал/приел за
ремонт“. Корекция изисква причина и създава нова версия, без overwrite на старата.
