...
QR кодът съдържа уеб адрес към `/machine/{id}`. След удостоверяване операторът получава mobile-friendly цифров паспорт с текуща наличност, активно предаване/ремонт, последен завършен ремонт, последно предаване, чакащи заявки за части, разрешени действия, технически полета, immutable история, прикачени снимки, ремонти, предавания, заявки, канонични и legacy протоколи и свързани технически документи. Подробният read-only договор и консолидацията на разделите са описани в [MACHINE_PASSPORT_V2_BG.md](docs/MACHINE_PASSPORT_V2_BG.md).
