...
- `backend/app/official_documents/registry.py` агрегира read-only каноничните и съвместимите historical transfer, repair и parts protocols. Той не генерира версии, не променя подписи/hash-ове и не записва audit; current official version има предимство пред legacy download редовете със същата domain identity. Backward-compatible пълният registry остава наличен, а category query пътят материализира само lightweight identity/chronology candidates за избраната категория, прилага literal identifier search и stable pagination и bulk-hydrate-ва domain, signature и file metadata само за текущата страница. Counts използва същата grouped/deduplicated identity логика без signature/file или binary hydration.
...
- `frontend/src/features/transfers/` притежава груповото издаване/връщане: `BulkTransfers.tsx` зарежда authoritative availability и историята, `IssueFlow.tsx` и `ReturnFlow.tsx` пазят отделните draft/confirmation/sign/result стъпки, а focused checklist, signature wrapper, summary, history, conflict и cancellation компоненти запазват същия markup. `transferState.ts` сериализира UI drafts без business authority; `transferApi.ts` използва непроменения authenticated клиент. Коренният `BulkTransfers.tsx` е explicit compatibility слой. Backend `transfer_service.py` и `transfer_signing.py` остават единственият авторитет за atomicity, status/location, manifest и signature completion. Вж. [component карта и regression/QA отчет](BULK_TRANSFERS_MODULARIZATION_BG.md).
