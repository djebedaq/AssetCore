...
Production Docker image включва LibreOffice Writer за PDF от exact filled DOCX source, DejaVu fonts, PostgreSQL client и минималните backup/verify/restore entry points за read-only one-shot Compose операции. След deploy проверете `/api/health`, Alembic head, owner designation, licence status и restore в отделна база. Изпълнимите non-root команди и правилата за външните mounts са в `BACKUP_RESTORE_BG.md`; вижте и `RELEASE_CHECKLIST_BG.md`.
