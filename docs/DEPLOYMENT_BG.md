...
Production Docker image включва LibreOffice Writer за PDF от exact filled DOCX source, DejaVu fonts, изричен PostgreSQL 16 client toolchain и минималните backup/verify/restore entry points за read-only one-shot Compose операции. Production DB остава PostgreSQL 16. Backup/restore и guarded upgrade отказват несъвместим major преди публикуване на архив или разрушително действие. Самостоятелните AES-GCM/checksum проверки не доказват възстановимост. След deploy проверете `/api/health`, Alembic head, owner designation, licence status и restore в отделна база. Изпълнимите non-root команди, PG16 договорът и правилата за външните mounts са в `BACKUP_RESTORE_BG.md`; ограниченият първи upgrade от baseline с PG17 clients е в `PORTABLE_PRODUCTION_DEPLOYMENT_BG.md`.
