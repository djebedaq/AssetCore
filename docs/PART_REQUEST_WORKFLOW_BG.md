...
Активна историческа заявка с дробно заявено или доставено количество се маркира от API с `quantity_compatibility.status=LEGACY_FRACTIONAL`, засегнатите `line_id` и детерминирано `recovery_action`. Тя не може да бъде подадена, одобрена или изпълнявана по новия целочислен процес. За `DRAFT` се създава нова заявка, като историческата чернова остава непроменена; за `WAITING_APPROVAL` се използва съществуващото отхвърляне; за `APPROVED`, `ORDERED` и `PARTIALLY_DELIVERED` се използва съществуващата отмяна с празен `lines[]`, след което се създава нова заявка с цели количества. Отмяната не променя историческите количества, приложенията, одита или вече съществуващите документи.
