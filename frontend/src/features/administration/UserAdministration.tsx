...
import { PASSWORD_MIN_LENGTH, passwordPolicyText } from '../../passwordPolicy'
import { storedUser } from '../../permissions'
import type { Department, ManagedUser, UserRole } from '../../types'

type Modal = 'create' | 'edit' | 'reset' | null

const emptyCreate = {
  email: '', first_name: '', middle_name: '', last_name: '', job_title: '', department_id: '',
  role: 'observer' as UserRole, preferred_language: 'bg',
...
export default function UserAdministration() {
  ...
  const [editForm, setEditForm] = useState({ first_name: '', middle_name: '', last_name: '', job_title: '', department_id: '', role: 'observer' as UserRole, preferred_language: 'bg' })
  ...
  async function load() {
    ...
    try {
      const [users, departmentItems] = await Promise.all([
        api<ManagedUser[]>(`/users${query.size ? `?${query}` : ''}`),
        api<Department[]>('/departments'),
      ])
      setItems(users)
      setDepartments(departmentItems.filter((item) => item.is_active))
    }
  ...
  async function createUser(event: FormEvent) {
    ...
    setError('')
    try {
      await api('/users', { method: 'POST', body: JSON.stringify({ ...createForm, department_id: createForm.department_id ? Number(createForm.department_id) : null }) })
      setCreateForm({ ...emptyCreate, preferred_language: session?.preferred_language || 'bg' })
  ...
  function openEdit(user: ManagedUser) {
    setError('')
    setSelected(user)
    setEditForm({ first_name: user.first_name || '', middle_name: user.middle_name || '', last_name: user.last_name || '', job_title: user.job_title || '', department_id: user.department_id ? String(user.department_id) : '', role: user.role, preferred_language: user.preferred_language })
    setModal('edit')
  }
  ...
  async function editUser(event: FormEvent) {
    ...
    setError('')
    try {
      await api(`/users/${selected.id}`, { method: 'PATCH', body: JSON.stringify({ ...editForm, department_id: editForm.department_id ? Number(editForm.department_id) : null }) })
  ...
  return <>
    ...
    {modal === 'create' && <UserModal title={t('users.add')} onClose={closeModal}><form className="form-grid" onSubmit={createUser}>{error && <div className="error wide" role="alert">{t(error)}</div>}<label>{t('profile.firstName')}<input required value={createForm.first_name} onChange={(event) => setCreateForm({ ...createForm, first_name: event.target.value })} /></label><label>{t('profile.middleName')}<input required value={createForm.middle_name} onChange={(event) => setCreateForm({ ...createForm, middle_name: event.target.value })} /></label><label>{t('profile.lastName')}<input required value={createForm.last_name} onChange={(event) => setCreateForm({ ...createForm, last_name: event.target.value })} /></label><label>{t('profile.jobTitle')}<input required value={createForm.job_title} onChange={(event) => setCreateForm({ ...createForm, job_title: event.target.value })} /></label><label>{t('profile.department')}<select value={createForm.department_id} onChange={(event) => setCreateForm({ ...createForm, department_id: event.target.value })}><option value="">{t('common.notSpecified')}</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name_bg}</option>)}</select></label><label>{t('users.workEmail')}<input type="email" required autoComplete="off" value={createForm.email} onChange={(event) => setCreateForm({ ...createForm, email: event.target.value })} /></label><label>{t('users.role')}<select value={createForm.role} onChange={(event) => setCreateForm({ ...createForm, role: event.target.value as UserRole })}>{availableRoles.map((value) => <option value={value} key={value}>{t(`role.${value}` as TranslationKey)}</option>)}</select></label><label>{t('users.language')}<select value={createForm.preferred_language} onChange={(event) => setCreateForm({ ...createForm, preferred_language: event.target.value as ManagedUser['preferred_language'] })}><option value="bg">{t('language.bg')}</option><option value="en">{t('language.en')}</option><option value="ru">{t('language.ru')}</option></select></label><label>{t('users.temporaryPassword')}<input required minLength={PASSWORD_MIN_LENGTH} type="password" autoComplete="new-password" value={createForm.temporary_password} onChange={(event) => setCreateForm({ ...createForm, temporary_password: event.target.value })} /></label><label>{t('users.confirmPassword')}<input required minLength={PASSWORD_MIN_LENGTH} type="password" autoComplete="new-password" value={createForm.confirm_password} onChange={(event) => setCreateForm({ ...createForm, confirm_password: event.target.value })} /></label><label className="check-label wide"><input type="checkbox" checked={createForm.is_active} onChange={(event) => setCreateForm({ ...createForm, is_active: event.target.checked })} />{t('users.activeAccount')}</label><p className="muted wide">{passwordPolicyText(session?.preferred_language)}</p><div className="actions wide"><button type="button" className="secondary" onClick={closeModal}>{t('common.cancel')}</button><button className="primary">{t('common.save')}</button></div></form></UserModal>}
    {modal === 'edit' && selected && <UserModal title={t('users.edit')} onClose={closeModal}><form className="form-grid" onSubmit={editUser}>{error && <div className="error wide" role="alert">{t(error)}</div>}<label>{t('profile.firstName')}<input required value={editForm.first_name} onChange={(event) => setEditForm({ ...editForm, first_name: event.target.value })} /></label><label>{t('profile.middleName')}<input required value={editForm.middle_name} onChange={(event) => setEditForm({ ...editForm, middle_name: event.target.value })} /></label><label>{t('profile.lastName')}<input required value={editForm.last_name} onChange={(event) => setEditForm({ ...editForm, last_name: event.target.value })} /></label><label>{t('profile.jobTitle')}<input required value={editForm.job_title} onChange={(event) => setEditForm({ ...editForm, job_title: event.target.value })} /></label><label>{t('profile.department')}<select value={editForm.department_id} onChange={(event) => setEditForm({ ...editForm, department_id: event.target.value })}><option value="">{t('common.notSpecified')}</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name_bg}</option>)}</select></label><label>{t('users.role')}<select value={editForm.role} onChange={(event) => setEditForm({ ...editForm, role: event.target.value as UserRole })}>{availableRoles.map((value) => <option value={value} key={value}>{t(`role.${value}` as TranslationKey)}</option>)}</select></label><label>{t('users.language')}<select value={editForm.preferred_language} onChange={(event) => setEditForm({ ...editForm, preferred_language: event.target.value as ManagedUser['preferred_language'] })}><option value="bg">{t('language.bg')}</option><option value="en">{t('language.en')}</option><option value="ru">{t('language.ru')}</option></select></label><div className="actions wide"><button type="button" className="secondary" onClick={closeModal}>{t('common.cancel')}</button><button className="primary">{t('common.save')}</button></div></form></UserModal>}
    {modal === 'reset' && selected && <UserModal title={t('users.resetPassword')} onClose={closeModal}><form className="form-grid" onSubmit={resetPassword}>{error && <div className="error wide" role="alert">{t(error)}</div>}<p className="wide">{t('users.resetFor', { name: selected.full_name })}</p><label>{t('users.temporaryPassword')}<input required minLength={PASSWORD_MIN_LENGTH} type="password" autoComplete="new-password" value={resetForm.temporary_password} onChange={(event) => setResetForm({ ...resetForm, temporary_password: event.target.value })} /></label><label>{t('users.confirmPassword')}<input required minLength={PASSWORD_MIN_LENGTH} type="password" autoComplete="new-password" value={resetForm.confirm_password} onChange={(event) => setResetForm({ ...resetForm, confirm_password: event.target.value })} /></label><p className="muted wide">{passwordPolicyText(session?.preferred_language)}</p><p className="muted wide">{t('users.forceChangeHint')}</p><div className="actions wide"><button type="button" className="secondary" onClick={closeModal}>{t('common.cancel')}</button><button className="primary">{t('users.resetPassword')}</button></div></form></UserModal>}
  </>
}
