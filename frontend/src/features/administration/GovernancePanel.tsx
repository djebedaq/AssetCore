...
export default function GovernancePanel({ session }: { session: UserSession }) {
  ...
  const hasVerifiedNonExpiringType = license
    && ['NOT_YET_VALID', 'ACTIVE', 'GRACE_PERIOD', 'READ_ONLY'].includes(license.state)
    && ['PERPETUAL', 'SUPPORT_ONLY'].includes(license.license_type || '')
