...
describe('production hardening workflows', () => {
  ...
  it('renders the three read-only registry sections with lifecycle and document actions', async () => {
    ...
    const incompleteRow = screen.getByText('TR-REG-009').closest<HTMLElement>('[data-registry-key]')
    ...
    expect(within(incompleteRow!).getByRole('button', { name: 'DOCX' })).toBeVisible()
    expect(within(incompleteRow!).getByRole('button', { name: 'PDF' })).toBeVisible()

    const completedRow = screen.getByText('TR-REG-010-R').closest<HTMLElement>('[data-registry-key]')
    ...
    expect(within(completedRow!).getAllByText('Протокол предаване')).toHaveLength(1)
    expect(within(completedRow!).getAllByText('Протокол приемане')).toHaveLength(1)

    const returnOnlyRow = screen.getByText('TR-REG-RETURN-ONLY-016-R').closest<HTMLElement>('[data-registry-key]')
