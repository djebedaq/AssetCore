...
export function MachinePassportModal({ machineId, onClose, onOpenCatalog }: { machineId: number; onClose: () => void; onOpenCatalog?: () => void }) {
  ...
  return (
    <Modal title={passport ? t('passport.title', { number: passport.machine.inventory_number }) : t('passport.loadingTitle')} onClose={onClose} wide>
      ...
      {!passport ? <div className="loading">{t('common.loading')}</div> : <>
        <div className="passport-hero">
          <img src={`/api/machines/${machineId}/qr`} alt={t('machines.qrAlt', { number: passport.machine.inventory_number })} />
        ...
        {tab === 'passport' && <div className="passport-grid">
          ...
          <section><h4>{t('passport.customFields')}</h4>{passport.custom_fields.map((field) => <label key={field.field_id}>{field[`label_${locale}` as 'label_bg'] || field.label_bg}{field.unit ? ` (${field.unit})` : ''}{customFieldControl(field)}</label>)}{!passport.custom_fields.length && <div className="empty-state">{t('passport.noCustomFields')}</div>}{can('admin') && passport.custom_fields.length > 0 && <button className="primary" onClick={saveCustomFields}>{t('common.save')}</button>}</section>
...
export function IndustrialCatalog({ defaultMachineId }: { defaultMachineId?: number } = {}) {
  ...
  return <>
    <div className="toolbar">
      ...
      {can('admin') && <button className="primary" onClick={() => setCreatePart(true)}><Plus size={17} />{t('catalog.addPart')}</button>}
    </div>
...
function PartHotspotViewer({ part, allParts, onRequest, onClose }: { part: CatalogPartEnhanced; allParts: CatalogPartEnhanced[]; onRequest: (part: CatalogPartEnhanced) => void; onClose: () => void }) {
  ...
  return <Modal title={`${part.part_number} · ${part.description}`} onClose={onClose} wide>
    ...
    <div className="hotspot-layout"><aside><dl className="detail-grid"><div><dt>{t('catalog.assembly')}</dt><dd>{part.assembly || t('common.noValue')}</dd></div><div><dt>{t('catalog.position')}</dt><dd>{part.position || t('common.noValue')}</dd></div><div><dt>{t('catalog.alternativeNumbers')}</dt><dd>{[part.alternative_part_number, ...(part.alternative_part_numbers || [])].filter(Boolean).join(', ') || t('common.noValue')}</dd></div><div><dt>{t('catalog.replacements')}</dt><dd>{replacements.map((item) => `${item.part_number} · ${item.description}`).join(', ') || t('common.noValue')}</dd></div><div><dt>{t('catalog.compatibility')}</dt><dd>{part.compatible_models || part.model || t('common.noValue')}</dd></div><div><dt>{t('catalog.specification')}</dt><dd>{part.technical_specification || t('common.noValue')}</dd></div><div><dt>{t('catalog.source')}</dt><dd>{part.source_document || t('common.noValue')} · {part.source_page || t('common.noValue')}</dd></div><div><dt>{t('catalog.confidence')}</dt><dd>{part.provenance_confidence == null ? t('common.noValue') : `${Math.round(part.provenance_confidence * 100)}%`}</dd></div></dl>
      ...
      {can('admin') && <><label>{t('catalog.hotspotProvenance')}<textarea value={source} onChange={(event) => setSource(event.target.value)} /></label><label>{t('catalog.hotspotConfidence')}<input type="number" min="0" max="1" step="0.01" value={confidence} onChange={(event) => setConfidence(event.target.value)} /></label><button className={marking ? 'primary' : 'secondary'} disabled={!documentId || !source.trim()} onClick={() => setMarking((value) => !value)}>{marking ? t('catalog.clickPosition') : t('catalog.addHotspot')}</button></>}
...
export function TechnicalLibrary() {
  ...
  return <><div className="toolbar"><div><h3>{t('documents.title')}</h3><p className="muted">{t('library.versionHint')}</p></div>{can('admin') && <button className="primary" onClick={() => setShowUpload('new')}><Upload size={17} />{t('library.upload')}</button>}</div><div className="filters"><div className="searchbox"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('library.searchPlaceholder')} /></div></div>{error && <div className="error">{error}</div>}<div className="library-grid">{items.map((document) => <article className="panel library-card" key={document.id}><div className="library-card-head"><BookOpen /><span><h4>{document.title}</h4><small>{document.brand} · {document.model || document.category}</small></span></div><div className="library-meta"><span>{t('library.revision')}: {document.revision || t('common.noValue')}</span><span>{t('library.versions', { count: document.revisions.length })}</span>{document.language && <span>{t(`language.${document.language}` as TranslationKey)}</span>}{document.page_count && <span>{t('library.pages', { count: document.page_count })}</span>}{document.linked_machine_numbers?.length ? <span>{t('library.linkedMachineCount', { count: document.linked_machine_numbers.length })}</span> : null}{document.tags?.map((tag) => <span className="badge" key={tag}>{tag}</span>)}{document.sha256 && <code>SHA-256 {document.sha256.slice(0, 12)}…</code>}</div>{document.source_label && <p className="muted">{t('library.source')}: {document.source_label}</p>}<div className="actions"><DownloadButton path={document.download_endpoint} filename={document.title} />{can('admin') && <button className="secondary compact" onClick={() => setShowUpload(document.id)}><Upload size={15} />{t('library.newRevision')}</button>}</div><details><summary>{t('library.versionHistory')}</summary>{document.revisions.map((revision) => <div className="revision-row" key={revision.id}><span><b>v{revision.version} · {revision.revision_label || t('common.noValue')}</b><small>{revision.change_note}</small></span><DownloadButton path={revision.download_endpoint} filename={revision.filename} /></div>)}</details></article>)}{!items.length && <div className="empty-state">{t('documents.empty')}</div>}</div>{showUpload && <TechnicalUploadModal document={revisionTarget} onClose={() => setShowUpload(null)} onSaved={() => { setShowUpload(null); void load() }} />}</>
}
...
export function AdministrationPanel() {
  ...
  const [error, setError] = useState('')
  const [modal, setModal] = useState<'user' | 'category' | 'field' | 'template' | 'location' | 'department' | null>(null)
  const [categoryId, setCategoryId] = useState<number | null>(null)
  const [userForm, setUserForm] = useState({ email: '', full_name: '', password: '', role: 'viewer', preferred_language: locale })
  ...
  const load = () => Promise.all([api<AdminUser[]>('/admin/users'), api<AssetCategory[]>('/categories'), api<DocumentTemplateInfo[]>('/document-templates'), api<AdminReferenceData>('/admin/reference-data')]).then(([userItems, categoryItems, templateItems, referenceData]) => { setUsers(userItems); setCategories(categoryItems); setTemplates(templateItems); setLocations(referenceData.locations); setDepartments(referenceData.departments); setError('') }).catch((caught) => setError(friendlyError(caught, t('admin.loadError'))))
  useEffect(() => { if (can('admin')) void load() }, [])
  if (!can('admin')) return null

  async function createUser(event: FormEvent) {
    event.preventDefault()
    try { await api('/admin/users', { method: 'POST', body: JSON.stringify(userForm) }); setModal(null); setUserForm({ email: '', full_name: '', password: '', role: 'viewer', preferred_language: locale }); await load() } catch (caught) { setError(friendlyError(caught, t('admin.saveError'))) }
  }
  async function toggleUser(user: AdminUser) {
    if (!window.confirm(t(user.is_active ? 'admin.deactivateConfirm' : 'admin.activateConfirm'))) return
    try { await api(`/admin/users/${user.id}`, { method: 'PATCH', body: JSON.stringify({ is_active: !user.is_active }) }); await load() } catch (caught) { setError(friendlyError(caught, t('admin.saveError'))) }
  }
