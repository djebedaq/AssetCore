...
import type { ExternalSigner, InternalParticipantCandidate, OfficialDocument } from './types'

const SLOT_CODES: Record<string, string[]> = {
  TRANSFER_ISSUE: ['HANDOVER', 'ACCEPTANCE'],
  TRANSFER_RETURN: ['RETURNED_BY', 'ACCEPTED_RETURN'],
  REPAIR_PROTOCOL: ['MECHANIC', 'ACCEPTANCE'],
  PART_REQUEST: ['REQUESTED_BY', 'APPROVED_BY'],
}
...
export default function OfficialDocuments() {
  ...
  ]).then(([documentValues, signerValues, candidateValues]) => {
  ...
  const slots = selected ? SLOT_CODES[selected.document_type] || [] : []
  ...
  async function assign() {
    if (!selected || slots.some((slot) => !assignments[slot])) return setError(t('official.selectAllSigners'))
    setBusy(true)
    setError('')
    try {
      await api(`/official-documents/${selected.id}/participants`, {
        ...
        body: JSON.stringify({ participants: slots.map((slot) => {
          const [kind, id] = assignments[slot].split(':')
          return { slot_code: slot, operation_role: slot, user_id: kind === 'internal' ? Number(id) : null, external_signer_id: kind === 'external' ? Number(id) : null }
        }) }),
      })
      await load()
    } catch (caught) {
  ...
  return (
    <>
      ...
      <div className="official-layout">
        ...
        {selected && <aside className="panel official-detail"><div className="panel-title"><h3>{selected.document_number}</h3><FileCheck2 /></div>{selected.current_version.status === 'DRAFT' ? <><p>{t('official.assignHint')}</p>{slots.map((slot) => <label key={slot}>{t(slotKey(slot))}<select value={assignments[slot] || ''} onChange={(event) => setAssignments((current) => ({ ...current, [slot]: event.target.value }))}><option value="">{t('common.notSpecified')}</option>{choices.map((choice) => <option key={`${slot}-${choice.value}`} value={choice.value}>{choice.label}</option>)}</select></label>)}<button className="primary" disabled={busy || !slots.length} onClick={assign}>{t('official.openForSigning')}</button></> : <><p>{t('official.participants')}</p><div className="participant-list">{selected.participants.map((participant) => <div key={participant.id}><div><strong>{String(participant.identity_snapshot.display_name || '')}</strong><span>{t(slotKey(participant.slot_code))} · {String(participant.identity_snapshot.job_title || '')}</span></div>{participant.signed ? <span className="badge success-badge">{t('official.signed')}</span> : <button className="secondary" disabled={busy} onClick={() => startSigning(participant.id)}>{t('official.createLink')}</button>}</div>)}</div>{selected.current_version.status === 'PARTIALLY_SIGNED' && <div className="notice">{t('official.partial')}</div>}{selected.current_version.status === 'SIGNED' && <div className="success">{t('official.fullySigned')}</div>}</>}</aside>}
      </div>
...
function ExternalSignerDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (value: ExternalSigner) => void }) {
  const { t } = useI18n()
  const [form, setForm] = useState({ first_name: '', middle_name: '', last_name: '', job_title: '', company: '', participant_role: '', note: '' })
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
...
function statusKey(value: string): TranslationKey {
  return ({ DRAFT: 'official.statusDraft', READY_FOR_SIGNATURE: 'official.statusReady', PARTIALLY_SIGNED: 'official.statusPartial', SIGNED: 'official.statusSigned', SUPERSEDED: 'official.statusSuperseded', CANCELLED: 'official.statusCancelled' } as Record<string, TranslationKey>)[value] || 'common.status'
}
function slotKey(value: string): TranslationKey {
  return ({ HANDOVER: 'official.slotHandover', ACCEPTANCE: 'official.slotAcceptance', RETURNED_BY: 'official.slotReturned', ACCEPTED_RETURN: 'official.slotReturnAcceptance', MECHANIC: 'official.slotMechanic', REQUESTED_BY: 'official.slotRequester', APPROVED_BY: 'official.slotApprover' } as Record<string, TranslationKey>)[value] || 'official.participantRole'
}
