...
export default function SignaturePage({ token, embedded = false, onFinished }: SignaturePageProps) {
  ...
  return (
    <main className={`signature-page ${embedded ? 'signature-embedded' : ''}`}>
      ...
      {summary && <section className="signature-summary"><h1>{t('signature.title')}</h1><dl>{summary.machine && <div><dt>{t('signature.machine')}</dt><dd>№{summary.machine.number} · {summary.machine.brand}</dd></div>}{summary.machines && summary.machines.length > 0 && <div><dt>{t('bulk.selectedSummary', { count: summary.machines.length })}</dt><dd>{summary.machines.map((machine) => `№${machine.number} · ${machine.brand}${machine.model ? ` ${machine.model}` : ''}`).join(', ')}</dd></div>}{summary.batch_reference && <div><dt>{t('bulk.batchLabel', { reference: summary.batch_reference })}</dt><dd>{summary.batch_reference}</dd></div>}<div><dt>{t('signature.operation')}</dt><dd>{summary.operation_description}</dd></div><div><dt>{t('signature.dateTime')}</dt><dd>{date(summary.operation_datetime)}</dd></div><div><dt>{t('signature.document')}</dt><dd>{summary.document_number}</dd></div><div><dt>{t('signature.version')}</dt><dd>{summary.document_version}</dd></div><div><dt>{t('signature.signer')}</dt><dd>{String(name || '')}</dd></div><div><dt>{t('signature.jobTitle')}</dt><dd>{String(job || '')}</dd></div><div><dt>{t('signature.role')}</dt><dd>{summary.operation_role}</dd></div></dl><small>SHA-256: {summary.batch_manifest_sha256 || summary.document_sha256}</small></section>}
