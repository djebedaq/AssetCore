...
def _add_official_repair_document(
    db, *, machine_id: int, repair_id: int, actor_id: int, number: str
) -> OfficialDocument:
...
def test_machine_scoped_official_documents_are_exact_and_canonical_wins(
    ...
    with session_factory() as db:
        ...
        db.flush()
        canonical = _add_official_repair_document(
            db,
            machine_id=target_id,
        ...
        for file_format in ("docx", "pdf"):
            content = f"legacy:{file_format}".encode()
            db.add(
                GeneratedDocument(
